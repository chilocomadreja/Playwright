...
test('test', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Setup' }).getByRole('button', { name: 'Setup', exact: true }).filter({ hasText: 'Setup' }).click();
  ...
  await page1.getByRole('tree', { name: 'Setup Tree' }).getByRole('group').getByRole('link', { name: 'Users' }).click();
  await page1.getByRole('heading', { name: 'Users' }).getByText('Users').click();
});