...
test.describe.parallel('CRM- Stworzenie Potencjalnego Klienta', () => {
 ...
 test('błędny nr tel. komórkowy', async ({ page }) => { 
  ...
  await page
    ...
    const komunikat4 = 'Pole tel.komórkowy może zawierać tylko cyfry lub powinno pozostać puste!Sprawdź ponownie te pole, a następnie wpisz właściwe dane.';
    expect(komunikat4).toContain('Pole tel.komórkowy może zawierać tylko cyfry lub powinno pozostać puste!Sprawdź ponownie te pole, a następnie wpisz właściwe dane.');
...
test('błędny adres mailowy', async ({ page }) => {
    ...
    await page
      ...
      const komunikat6 = 'Napotkaliśmy nieoczekiwaną przeszkodę. Sprawdź następujące pola';
      expect(komunikat6).toContain('Napotkaliśmy nieoczekiwaną przeszkodę. Sprawdź następujące pola');
